...
`appclone doctor` emits findings using the stable codes below. Each finding has a severity (`info`/`warn`/`error`), a category, one or more pieces of evidence, and a short suggested fix.
...
`codesign --verify --deep --strict` reports the clone's signature is not self-consistent. The most common cause is a nested signable (framework, helper app, XPC service) that was missed during discovery or signed out of order. Re-run `appclone doctor` after a clone to see the specific failure; if appclone's discovery is missing a real signable directory, that's a bug to report.
...
A `Sparkle.framework` was detected. Sparkle validates updates against the app's original bundle identity and signature. Updates on the clone will either fail silently, refuse to apply, or (worst case) clobber your bundle ID back to the original's. Either disable Sparkle's auto-update in the clone's preferences, or expect to re-run `appclone` after each source-app update.

### `electron_helper` — severity: **info** — category: helper
An `Electron Framework.framework` was detected. Electron apps embed the parent bundle ID into each helper's Info.plist (e.g. `com.foo.app.helper (Renderer)`). appclone rewrites these automatically so the clone stays internally consistent. No action needed — this finding exists so you know why helper bundle IDs look different after cloning.
